...
for J in range(1,8):
    for skip_hps in (False, True):
        ...
        level2plus = "\n".join(
            [level2plus_fwd_inv.format(j=j, j2=j-1) for j in range(J,1,-1)])
        ...
        level2plusbwd = '\n'.join(
            [level2plus_bwd.format(j=j, j2=j-1) for j in range(J,1,-1)])
