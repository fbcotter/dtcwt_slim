...
from dtcwt_slim.torch.lowlevel import coldfilt, prep_filt
...
def test_equal_small_in():
    ...
    y = coldfilt(im_t, prep_filt(ha, ch), prep_filt(hb, ch), np.sum(ha*hb)>0)
...
def test_equal_numpy_qshift1():
    ...
    y = coldfilt(barbara_t, prep_filt(ha, ch), prep_filt(hb, ch), np.sum(ha*hb)>0)
    np.testing.assert_array_almost_equal(y[0], ref, decimal=4)
...
def test_gradients():
    ...
    y_t = coldfilt(im_t, prep_filt(ha, ch), prep_filt(hb, ch), np.sum(ha*hb)>0)
