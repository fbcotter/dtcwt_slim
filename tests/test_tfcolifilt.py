...
from dtcwt_slim.tf.lowlevel import colifilt
...
def setup():
    ...
    global ref_colifilt, t_shape, config
    ...
    ref_colifilt = lambda x, ha, hb: np.stack(
        [np_colifilt(s, ha, hb) for s in x], axis=0)
...
def test_odd_filter():
    with raises(ValueError):
        colifilt(barbara_t, (-1,2,-1), (-1,2,1))


def test_different_size_h():
    with raises(ValueError):
        colifilt(barbara_t, (-1,2,1), (-0.5,-1,2,-1,0.5))


def test_zero_input():
    Y = colifilt(barbara_t, (-1,1), (1,-1))
...
def test_bad_input_size():
    with raises(ValueError):
        colifilt(barbara_t[:,:, :511,:], (-1,1), (1,-1))
...
def test_output_size_non_mult_4():
    Y = colifilt(barbara_t, (-1,0,0,1), (1,0,0,-1))
    assert Y.shape[1:] == bshape_double
...
def test_equal_numpy_qshift1():
    ...
    ref = ref_colifilt(barbara, ha, hb)
    y_op = colifilt(barbara_t, ha, hb)
    with tf.Session(config=config) as sess:
        y = sess.run(y_op)
    np.testing.assert_array_almost_equal(y[0], ref, decimal=4)
