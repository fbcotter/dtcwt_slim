...
from dtcwt_slim.tf.lowlevel import rowdfilt
...
def setup():
    ...
    global ref_rowdfilt, t_shape, config
...
def test_odd_filter():
    with raises(ValueError):
        rowdfilt(barbara_t, (-1,2,-1), (-1,2,1))


def test_different_size():
    with raises(ValueError):
        rowdfilt(barbara_t, (-0.5,-1,2,1,0.5), (-1,2,-1))


def test_bad_input_size():
    with raises(ValueError):
        rowdfilt(barbara_t[:,:,:,:511], (-1,1), (1,-1))
...
def test_output_size():
    y_op = rowdfilt(barbara_t, (-1,1), (1,-1))
